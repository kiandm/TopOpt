...
for e = check_elems % 1:numele
   ...
   xphy_pos = apply_filter_and_projection(xval_pos, numele, H, Hs, beta, eta);
   xphy_neg =  apply_filter_and_projection(xval_neg, numele, H, Hs, beta, eta);
   [U_pos, ~, ~,~] = FE_analysis(xphy_pos, penal, numnode, numele, gs, edofMat, coords, conn, freedofs, F, matprop, dphix_ref, dphiy_ref);
   [U_neg, ~, ~,~] = FE_analysis(xphy_neg, penal, numnode, numele, gs, edofMat, coords, conn, freedofs, F, matprop, dphix_ref, dphiy_ref);
   [c_pos, ~, ~] = objective_function(U_pos, xphy_pos, penal, numele, gs, edofMat, coords, conn, matprop, dphix_ref, dphiy_ref);
   [c_neg, ~, ~] = objective_function(U_neg, xphy_neg, penal, numele, gs, edofMat, coords, conn, matprop, dphix_ref, dphiy_ref);
