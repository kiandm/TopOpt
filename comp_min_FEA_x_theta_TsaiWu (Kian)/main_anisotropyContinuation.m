...
% Anisotropy continuation from initial isotropic optimisation to fix local
% minima for theta
clear; clc; close all; 
warning off
%% Parameters
...
alpha_ramp_end = 200; % Iteration number to reach full anisotropy
%%
...
while change > 1e-3 && iter < maxiter
    ...
    [g_tw, dgtw_dx, dgtw_dtheta, TW, TW_gp, vonMises] = TsaiWu(U, K, KE0, xphy, penal, numele, gs, edofMat, coords, conn, matprop_c, strength, freedofs);
    % [g_tw, dgtw_dx, dgtw_dtheta, TW, vonMises] = Copy_of_TsaiWu(U, K, KE0, xphy, penal, numele, gs, edofMat, coords, conn, matprop, strength, freedofs);
    % Objective function and sensitivities
    [c, dc_dx, dc_theta] = objective_function(U, xphy, penal, numele, gs, edofMat, coords, conn, matprop_c);    
    ...
    dc_theta    = H*(dc_theta./Hs);                % ADD THESE BACK IN AND FIX
    ...
    xphy(1:numele)     = (H*xval(1:numele))./Hs;
    % xphy(numele+1:end) = (H*xval(numele+1:end))./Hs; % ADD FILTER BACK IN HERE
    % xval=xphy; 
    % Print results
    ...
    fprintf('It %d: alpha=%.3f, Obj = %f, V = %f, g_tw = %f, Change = %f\n', iter, alpha, c, v, g_tw, change);
    ...
    if mod(iter, 5) == 0 || iter == 1
        ...
        hold on;
        ind = find(xphy(1:numele) > 0.1); % Only show fibers where there is material
...
warning on
%% Final solve with true composite properties for post-processing plots
% [U_final, ~, ~] = FE_analysis(xphy, penal, numnode, numele, gs, ...
%                                edofMat, coords, conn, freedofs, F, matprop);
% [~, ~, ~, TW, ~, vonMises] = TsaiWu(U_final, [], [], xphy, penal, ...
%     numele, gs, edofMat, coords, conn, matprop, strength, freedofs);
%%
% Measures of non-discreteness
