...
dbstop if error
% Parameters-----------------------------------------------
volfrac = 0.25; penal = 3.0; rmin = 2; E0 = 1.0; 
Emin = 1e-4; maxit = 248; tol = 1e-3; nu = 0.3; 
k_in = 1; k_out = 0.001;
beta = 8; eta = 0.5;
De = E0/(1-nu^2) * [1, nu,     0; 
                  nu,  1,     0; 
...
v = VideoWriter('topology_optimisation_2.mp4', 'MPEG-4');
...
for it = 1:maxit
    ...
    %x_tilde = applySensitivityFilter(x, x, Wi, Wj, Ww, Wsum);
    ...
    % Storing x values for specific iteration plotting
    %storage(it) = [x_phy];
    % Track best design
    if c < c_best
        c_best = c;
...
close(v);
close(fig);
fprintf('Video saved as topology_optimisation.mp4\n');
%%
...
% M = sum(4 * x_phy * (1 - x_phy))/n * 100
%fprintf('Measure of non-discrete, M = %6.4f', M)
...
figure;
plotDensityMesh(nodes, enodes, x_phy_best);
xlabel('X Coordinate');
ylabel('Y Coordinate');
title('%6.4f Design Density', it_best);
colorbar;
grid on;
