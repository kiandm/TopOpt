...
beta = 2;      % Heaviside curvature parameter
eta = 0.5;
De = E0/(1-nu^2) * [1, nu,     0; 
                  nu,  1,     0; 
...
v = VideoWriter('topology_optimisation_6.mp4', 'MPEG-4');
...
for it = 1:maxit
    ...
    if mod(it, 50) == 0 && beta < 10
        beta = min(beta + 0.4, 10);
...
xlabel('X Coordinate');
ylabel('Y Coordinate');
title(sprintf('Best Design at Iteration %d (c=%.4f)', it_best, c_best));
colorbar;
grid on;
