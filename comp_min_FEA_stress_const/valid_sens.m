...
while change > 1e-3 && iter < 100 
    ...
    if iter==20 % calculate sensitivities at this iteration


        figure(1)
        subplot(2,1,1)
        colormap(jet);
        patch('Faces',conn','Vertices',coords','FaceVertexCData',dgmax_x,...
              'FaceColor','flat','EdgeColor',[0,0,0]); axis equal off; colorbar;  
    
        figure(2)
        subplot(2,1,1)
        colormap(jet);
        patch('Faces',conn','Vertices',coords','FaceVertexCData',dgmax_theta,...
              'FaceColor','flat','EdgeColor',[0,0,0]); axis equal off; colorbar; 

        %origianl sensitivities
        gmax0=gmax_con+1; 

         % Perturb sensitivities
         h=1e-6; 
         for ee=1:numele
             fprintf('ee %d: of element %d\n', ee, numele);

             %x
             xphy1=xphy; 
             xphy1(ee)=xphy(ee)+h;
            [gpn, gmax_con, ge, dgmax_x, dgmax_thet, cI] = stress_const(xphy1, coords, conn, ...
                matprop, pl, p, s_pl, gs, gs1, numele, U, edofMat, K, numnode,freedofs,cI_1,alpha_I,gpn_I_1,ge_I_1,H,Hs);   
            gmax1=gmax_con+1;
            dgmax_x_fd(ee)=(gmax1-gmax0)/h;

            %theta
             xphy1=xphy; 
             xphy1(numele+ee)=xphy(numele+ee)+h;
            [gpn, gmax_con, ge, dgmax_x, dgmax_thet, cI] = stress_const(xphy1, coords, conn, ...
                matprop, pl, p, s_pl, gs, gs1, numele, U, edofMat, K, numnode,freedofs,cI_1,alpha_I,gpn_I_1,ge_I_1,H,Hs);   
            gmax1=gmax_con+1;
            dgmax_theta_fd(ee)=(gmax1-gmax0)/h;            
         end

        figure(1)
        subplot(2,1,2)
        colormap(jet);
        patch('Faces',conn','Vertices',coords','FaceVertexCData',dgmax_x_fd',...
              'FaceColor','flat','EdgeColor',[0,0,0]); axis equal off; colorbar;  
    
        figure(2)
        subplot(2,1,2)
        colormap(jet);
        patch('Faces',conn','Vertices',coords','FaceVertexCData',dgmax_theta_fd',...
              'FaceColor','flat','EdgeColor',[0,0,0]); axis equal off; colorbar; 
    end 
    ...
    % fval = [v_con]; % volume and stress constraint value
    ...
    % dfdx = [dv_dx'   dv_theta']; % Volume and stress constraint sensitivities (transpose)
