...
for it = 1:maxit
    ...
    % ----- fmincon update -------------------
    % Use fmincon to solve: minimize compliance c(x) subject to volume and bounds
    % Variables: x (nel x 1)
    nelm = numel(x);
    % Objective wrapper: compute compliance given x (ensure column)
    function [cf, gradf] = obj_fun(xvar)
        xvar = max(min(xvar,1),0); % keep feasible numerically
        ...
        % apply sensitivity filter (consistent with OC case)
        dcf_local = applySensitivityFilter(dcf_local, xvar, Wi, Wj, Ww, Wsum);
        gradf = dcf_local;
    end
    % Nonlinear constraint: volume equality (or inequality). Use equality to match volfrac.
    function [cineq, ceq, gradcineq, gradceq] = vol_con(xvar)
        ceq = mean(xvar) - volfrac;
        cineq = [];
        if nargout > 2
            gradceq = (1/nelm) * ones(nelm,1);
            gradcineq = [];
        end
    end
    % fmincon options
    opts = optimoptions('fmincon', 'Algorithm', 'interior-point', ...
        ...
        'Display', 'none', 'MaxIterations', 50, 'OptimalityTolerance', 1e-3);
    % Bounds
    lb = zeros(nelm,1); ub = ones(nelm,1);
    % Initial guess: current x
    try
        xfc = fmincon(@obj_fun, x, [], [], [], [], lb, ub, @vol_con, opts);
        x = xfc;
    catch
        % If fmincon fails for any reason, fallback to OC update
        x = OC_update(x, dcf, volfrac);
    end
    % x = fmincon()
    % ----- Convergence, logging -------------
