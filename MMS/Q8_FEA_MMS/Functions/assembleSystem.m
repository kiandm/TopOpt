...
function [KE, FE] = assembleElement(e, nodes, enodes, edof, De, mms, gp, w, shapeFcn)
    ...
    FE = zeros(16,1);
    % For each of the 3 gauss points and weights define the natural coords
    for ixi = 1:1:length(gp)
        xi = gp(ixi); wx = w(ixi);
        for ieta = 1:1:length(gp)
            eta = gp(ieta); wy = w(ieta);
            % Define the derivatives of the 8 quadratic shape functions
            ...
            dNdx = J \ dNdxi; % 2x4
            % B matrix (3x16)
            % B=[B1 B2 B3 B4 B5 B6 B7 B8] where each Bi is 3x2 -> 3x16
            B = zeros(3,16); % Initialise
            for i = 1:8    % Loop over the 8 nodes of the element
